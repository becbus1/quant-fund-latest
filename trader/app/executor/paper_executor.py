...
class PaperExecutor:
    ...
    def __init__(self):
        ...
        # IMPORTANT:
        # Store position IDs, NOT ORM objects
    ...
    def execute_entry(
        ...
        if side == Side.BUY:
            tp = fill_price * (1 + take_profit_bps / 10000)
            sl = fill_price * (1 - stop_loss_bps / 10000)
        else:
            tp = fill_price * (1 - take_profit_bps / 10000)
            sl = fill_price * (1 + stop_loss_bps / 10000)
    ...
    def execute_exit(
        ...
        logger.info(
            f"Exit fill: {exit_side.value} {symbol} @ {fill_price:.4f} "
